data<-dlt
count<-dim(dlt)[1]
#dlt.SVMFIII <- function(data,count) {
library(corrplot)
...
svmFit.a1<-train(resa1~a1.1+a2.1+a3.1+a4.1+a5.1+
                 ...
                 trControl = trainControl(method = "repeatedcv",
                                          repeats = n.r))
plot(svmFit.a1,scales = list(x = list(log = 2)),main = "a1")
...
testPredictions.a1<-predict(svmFit.a1,tests.ab)
testPredictions.a2<-predict(svmFit.a2,tests.ab)
testPredictions.a3<-predict(svmFit.a3,tests.ab)
testPredictions.a4<-predict(svmFit.a4,tests.ab)
testPredictions.a5<-predict(svmFit.a5,tests.ab)
testPredictions.b1<-predict(svmFit.b1,tests.ab)
testPredictions.b2<-predict(svmFit.b2,tests.ab)

result.svm<-c(tail(testPredictions.a1,1),
