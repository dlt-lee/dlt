...
dlt.pr.xgb.II <- function(data,count,seed) {
  ...
  train.T.a1<-Matrix(as.matrix(train.a1[,4:17]),sparse=T)
  train.T.a2<-Matrix(as.matrix(train.a2[,4:17]),sparse=T)
  train.T.a3<-Matrix(as.matrix(train.a3[,4:17]),sparse=T)
  train.T.a4<-Matrix(as.matrix(train.a4[,4:17]),sparse=T)
  train.T.a5<-Matrix(as.matrix(train.a5[,4:17]),sparse=T)
  train.T.b1<-Matrix(as.matrix(train.b1[,4:17]),sparse=T)
  train.T.b2<-Matrix(as.matrix(train.b2[,4:17]),sparse=T)
