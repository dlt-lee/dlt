dlt.xgb.row.9<-function(trains,d_o) {
  ...
  if (threads <= 8) {
    ...
    bst.a4<-xgboost(data = trains.T,label = trains$res.a4,nrounds = 300,verbose=0,params = list(tree_method = t_m))
    bst.a5<-xgboost(data = trains.T,label = trains$res.a5,nrounds = 300,verbose=0,params = list(tree_method = t_m))
    bst.b1<-xgboost(data = trains.T,label = trains$res.b1,nrounds = 300,verbose=0,params = list(tree_method = t_m))
    bst.b2<-xgboost(data = trains.T,label = trains$res.b2,nrounds = 300,verbose=0,params = list(tree_method = t_m))
    ...
    bst.b2<-xgboost(data = trains.T,label = trains$res.b2,nrounds = 300,verbose=0,params = list(tree_method = t_m,nthread=number_of_core))
