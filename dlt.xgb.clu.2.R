dlt.xgb.clu.2<-function(trains,d_o) {
  ...
  bst.a1<-xgboost(data = trains.T[,c(50,57)],label = trains$res.a1,nrounds = 300,print_every_n = 300L,params = list(tree_method = 'hist'))
  bst.a2<-xgboost(data = trains.T[,c(51,58)],label = trains$res.a2,nrounds = 300,print_every_n = 300L,params = list(tree_method = 'gpu_hist'))
  bst.a3<-xgboost(data = trains.T[,c(52,59)],label = trains$res.a3,nrounds = 300,print_every_n = 300L,params = list(tree_method = 'hist'))
  bst.a4<-xgboost(data = trains.T[,c(53,60)],label = trains$res.a4,nrounds = 300,print_every_n = 300L,params = list(tree_method = 'gpu_hist'))
  bst.a5<-xgboost(data = trains.T[,c(54,61)],label = trains$res.a5,nrounds = 300,print_every_n = 300L,params = list(tree_method = 'hist'))
  bst.b1<-xgboost(data = trains.T[,c(55,62)],label = trains$res.b1,nrounds = 300,print_every_n = 300L,params = list(tree_method = 'gpu_hist'))
  bst.b2<-xgboost(data = trains.T[,c(56,63)],label = trains$res.b2,nrounds = 300,print_every_n = 300L,params = list(tree_method = 'hist'))
