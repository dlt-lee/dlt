dlt.xgb.row.6<-function(trains,d_o) {
  ...
  bst.a1<-xgboost(data = trains.T,label = trains$res.a1,nrounds = 300,print_every_n = 300L,params = list(tree_method = 'gpu_hist'))
  ...
  bst.b2<-xgboost(data = trains.T,label = trains$res.b2,nrounds = 300,print_every_n = 300L,params = list(tree_method = 'gpu_hist'))
