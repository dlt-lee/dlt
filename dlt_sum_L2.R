dlt_sum_L2<-function(data_org,n) {
  ...
  bst.a2<-xgboost(data = trains.T.a2,label = res_m[,2],nrounds = 300,print_every_n = 300L,params = list(tree_method = 'gpu_hist'))
  ...
  bst.b1<-xgboost(data = trains.T.b1,label = res_m[,6],nrounds = 300,print_every_n = 300L,params = list(tree_method = 'gpu_hist'))
