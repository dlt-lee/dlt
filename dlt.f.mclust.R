library(mclust)
dlt.f.mclust<-function(mytable) {
  
  #Build data
  tests<-tail(mytable,1)
  trains.a <-tail(mytable,201)[1:200,]
  results.a<-tail(mytable,200)
  #A:
  trn<-trains.a$n
  a1<-trains.a$a1
  a2<-trains.a$a2
  ...
  p.a1<-sort(p.a1,decreasing = TRUE)
  nm.a1<-as.data.frame.integer(names(p.a1))[,1]
  p.a1<-as.data.frame(p.a1)$p.a1
  p.a1<-data.frame(nm.a1,p.a1)
  
  p.a2<-sort(p.a2,decreasing = TRUE)
  nm.a2<-as.data.frame.integer(names(p.a2))[,1]
  p.a2<-as.data.frame(p.a2)$p.a2
  p.a2<-data.frame(nm.a2,p.a2)
  
  p.a3<-sort(p.a3,decreasing = TRUE)
  nm.a3<-as.data.frame.integer(names(p.a3))[,1]
  p.a3<-as.data.frame(p.a3)$p.a3
  p.a3<-data.frame(nm.a3,p.a3)
  
  p.a4<-sort(p.a4,decreasing = TRUE)
  nm.a4<-as.data.frame.integer(names(p.a4))[,1]
  p.a4<-as.data.frame(p.a4)$p.a4
  p.a4<-data.frame(nm.a4,p.a4)
  
  p.a5<-sort(p.a5,decreasing = TRUE)
  nm.a5<-as.data.frame.integer(names(p.a5))[,1]
  p.a5<-as.data.frame(p.a5)$p.a5
  p.a5<-data.frame(nm.a5,p.a5)
  
  p.b1<-sort(p.b1,decreasing = TRUE)
  nm.b1<-as.data.frame.integer(names(p.b1))[,1]
  p.b1<-as.data.frame(p.b1)$p.b1
  p.b1<-data.frame(nm.b1,p.b1)
  
  p.b2<-sort(p.b2,decreasing = TRUE)
  nm.b2<-as.data.frame.integer(names(p.b2))[,1]
  p.b2<-as.data.frame(p.b2)$p.b2
  p.b2<-data.frame(nm.b2,p.b2)
  
  p<-c(p.a1,p.a2,p.a3,p.a4,p.a5,p.b1,p.b2)
  return(p)
